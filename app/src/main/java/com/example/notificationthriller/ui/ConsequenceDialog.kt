...
import com.example.notificationthriller.data.ConsequenceNotification
import com.example.notificationthriller.data.ConsequenceType
...
object ConsequenceDialog {
     ...
     */
    fun show(context: Context, notification: ConsequenceNotification, onDismiss: (() -> Unit)? = null) {
     ...
     */
    fun showEndingUnlocked(context: Context, endingName: String, description: String) {
        val builder = AlertDialog.Builder(context)
    ...
    private fun getColorForType(type: ConsequenceType): Int {
        return when (type) {
            ...
            ConsequenceType.SECRET_REVEALED -> Color.parseColor("#4CAF50") // Green
            
            ConsequenceType.TRUST_LOST,
            ConsequenceType.RELATIONSHIP_DAMAGED,
            ConsequenceType.ENDING_CLOSED,
            ConsequenceType.DANGER_INCREASED -> Color.parseColor("#F44336") // Red
